...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=51e4b99a8c3e4663969f72091152e8dd';
...
$(document).ready(() => {
  ...
  for (let i in links) {
    ...
    $('#marquee').append(`<a href="https://e-z.bio/${link.link}" target="_BLANK">${link.name}</a>`);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[chep]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>www.chep.buzz $$$$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
